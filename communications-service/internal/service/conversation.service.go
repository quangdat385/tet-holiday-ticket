...
type (
	IConversationService interface {
		...
		GetConversationByUserId(context context.Context, user_id int64, query vo.PageRequest) (out []model.ConversationOutput, err error)
		...
		AddUserToConversation(context context.Context, conversationId int64, userIds []model.UserIDS) (out model.ConversationOutput, err error)
		...
		SoftDeleteConversation(context context.Context, in model.SoftDeleteConversationInput) (out bool, err error)
	}
)
