...
func (c *ConversationRouter) InitConversationRouter(Router *gin.RouterGroup) {
	...
	ConversationRouterPrivateGroup.Use(middleware.AuthenMiddleWare(), middleware.RoleMiddleware("User"))
	{
		...
		ConversationRouterPrivateGroup.PUT("soft-delete/:conversation_id", controller.ConversationControllerRouter.SoftDeleteConversation)
	}
}
